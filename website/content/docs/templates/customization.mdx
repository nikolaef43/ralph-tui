...
<Callout type="info">
This template includes **compound learning** - agents document learnings that persist across iterations via `{{recentProgress}}` and `{{codebasePatterns}}`, making each iteration smarter than the last.
</Callout>

### Frontend-Focused Template

For UI development with browser verification (includes PRD context):
...
For backend/API work (includes PRD context):
